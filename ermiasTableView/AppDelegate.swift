...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let controller = navigationController.topViewController as! ErmiasTableViewController
    ...
    func storesWillChange(notification: NSNotification) {
        ...
        println("storesWillChange was executed")

        var error:NSError
        if(context.hasChanges){
            var success:Bool = context.save(nil)
            if(!success){
                println("No success")
            }else{
                 println("Store Will Change: success")
                let navigationController = self.window!.rootViewController as! UINavigationController
                let controller = navigationController.topViewController as! ErmiasTableViewController
                controller.managedObjectContext = context
                controller.fetchedResultsController.managedObjectContext.mergeChangesFromContextDidSaveNotification(notification)
            }
        }
        context.reset()
    ...
    func storesDidChange(notification: NSNotification) {
        ...
        println("storesDidChange was executed")
        let navigationController = self.window!.rootViewController as! UINavigationController
        let controller = navigationController.topViewController as! ErmiasTableViewController
        //controller.tableView.reloadData()
       ...
       println("Get new Data!")
        ...
        dispatch_async(dispatch_get_global_queue(Int(QOS_CLASS_USER_INITIATED.value), 0)) { // 1
            ...
            dispatch_async(dispatch_get_main_queue()) { // 2
                context.mergeChangesFromContextDidSaveNotification(changeNotification) // 3
            }
        }
                //controller.tableView.reloadData()
      /*
    ...
    private func updateUserDefaultsFromICloud() {
            ...
            println(values)
            if values["text"] != nil {
                 println("Get update \(values.count)")
            let selectedColorIndex =
                Int(NSUbiquitousKeyValueStore.defaultStore().longLongForKey(
            "text"))
            let prefs = NSUserDefaults.standardUserDefaults()
            prefs.setInteger(selectedColorIndex, forKey: "text")
            prefs.synchronize()
            }
    }
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "fi.yea.ermiasTableView" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("ermiasTableView", withExtension: "momd")!
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
       ...
       // let storeOptions:NSDictionary = NSPersistentStoreUbiquitousContentNameKey;
        var storeOptions =
        [NSPersistentStoreUbiquitousContentNameKey : "ermiasTableView"
            // NSPersistentStoreRebuildFromUbiquitousContentOption: @(true)
        ]
        
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("ermiasTableView.sqlite")
